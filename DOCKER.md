...
```sh
docker build -t cloudinary-media-generation-mcp .
```
...
```sh
docker build -t cloudinary-media-generation-mcp https://github.com/cloudinary/media-generation-mcp.git
```
...
docker run -d -p 2718:2718 \
  ...
  cloudinary-media-generation-mcp start --transport sse
```
...
```sh
docker run -d -p 2718:2718 -e CLOUDINARY_URL cloudinary-media-generation-mcp start --transport sse
```
...
To stop the container started from the `cloudinary-media-generation-mcp` image:
```sh
docker stop $(docker ps -a -q --filter "ancestor=cloudinary-media-generation-mcp")
```
...
docker run -d -p 2718:2718 \
  -e CLOUDINARY_URL \
  cloudinary-media-generation-mcp start --transport sse --log-level debug
```
